...
def log_event(dog_id: int, payload: EventCreate, db: Session = Depends(get_db)):
...
def list_events(dog_id: int, limit: int = 100, db: Session = Depends(get_db)):
    _get_dog_or_404(dog_id, db)
    return (
        db.query(Event)
...
def delete_event(dog_id: int, event_id: int, db: Session = Depends(get_db)):
